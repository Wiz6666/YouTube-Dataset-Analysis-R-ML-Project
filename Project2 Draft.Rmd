...
# 使用fluidPage构建一个流式页面
ui_model <- fluidPage(
  # 设置页面的标题为"Model Selection"
  titlePanel("Model Selection"),
  
  # 使用侧边栏布局，包括一个侧边栏面板和一个主面板
  sidebarLayout(
    # 侧边栏面板
    sidebarPanel(
      # 创建一个下拉选择框，供用户选择模型类型
      ...
      # 当选择的模型类型为"Decision Tree Model"时，显示此条件面板
      conditionalPanel(
      ...
      # 当选择的模型类型为"Logistic Regression Model"时，显示此条件面板
      conditionalPanel(
      ...
      # 当选择的模型类型为"Random Forest Model"时，显示此条件面板
      conditionalPanel(
