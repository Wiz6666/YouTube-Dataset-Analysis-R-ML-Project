...
youtube_data$video_views_increasing_rate <- (youtube_data$video_views_last_30_days/youtube_data$video_views)*100
youtube_data$video_views_increasing_rate <- sprintf("%.4f%%", youtube_data$video_views_increasing_rate)
```
...
```
### 4.3.2 Select suitable single variable
...
So we use a function to find which features' AUC values are significantly better than Null model
```{r}
