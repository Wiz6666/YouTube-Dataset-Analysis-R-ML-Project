...
calculateLogLikelihood <- function(model, x, y) {
  # 获取模型的预测概率
  predicted_probabilities <- predict(model, newdata = x)
  
  # 计算对数似然
  log_likelihood <- sum(ifelse(y == 1, log(predicted_probabilities), log(1 - predicted_probabilities)))
...
cat('This model\'s AUC score on the training set is',tmodel1_train_auc, "\n")

#test set AUC score
tmodel1_test_auc <- AUC_calculator(predict(tmodel1, newdata=youtube_test), youtube_test[,target_column])
cat('This model\'s AUC score on the testing set is',tmodel1_test_auc)
```
...
#predicted_probabilities <- predict(tmodel1, newdata = youtube_test)
cat("tmodel1 Log Likelihood:", calculateLogLikelihood(tmodel1, youtube_test, positive_label), "\n")
...
cat('This model\'s AUC score on the training set is',tmodel2_train_auc, "\n")

#test set AUC score
tmodel2_test_auc <- AUC_calculator(predict(tmodel2, newdata=youtube_test), youtube_test[,target_column])
cat('This model\'s AUC score on the testing set is',tmodel2_test_auc)
```
...
cat("tmodel2 Log Likelihood:", calculateLogLikelihood(tmodel2, youtube_test, positive_label), "\n")
```
##### d. Print the ROC plot
```{r}
...
cat('This model\'s AUC score on the training set is',tmodel3_pre_train_auc, "\n")

#test set AUC score
tmodel3_pre_test_auc <- AUC_calculator(predict(tmodel3, newdata=youtube_test), youtube_test[,target_column])
cat('This model\'s AUC score on the testing set is',tmodel3_pre_test_auc)
```
##### c. Print the performance table
```{r warning=FALSE}
...
cat("tmodel3 Log Likelihood:", calculateLogLikelihood(tmodel3, youtube_test, positive_label), "\n")
```
##### d. Print the ROC plot
```{r}
...
performance
```{r}
cat("lrmodel1 Log Likelihood:", calculateLogLikelihood(lrmodel1, youtube_test, positive_label), "\n")
```
...
performance
```{r}
cat("lrmodel2 Log Likelihood:", calculateLogLikelihood(lrmodel2, youtube_test, positive_label), "\n")
```
...
performance
```{r}
cat("lrmodel3 Log Likelihood:", calculateLogLikelihood(lrmodel3, youtube_test, positive_label), "\n")
```
...
### 4.6.1 Ensure features are independent of each other
```{r}
library(caret)

# 创建一个通用的函数来消除特征组合中的相互关系
eliminate_feature_relations <- function(data, feature_combinations) {
  # 从数据中提取每个特征组合的特征
  combined_features <- lapply(feature_combinations, function(combination) {
    data[, combination]
  })

  # 连接特征组合以创建一个新数据框
  combined_data <- do.call(cbind, combined_features)

  # 使用主成分分析 (PCA) 来将特征组合投影到新的正交特征空间
  pca_result <- prcomp(combined_data, center = TRUE, scale. = TRUE)

  # 从PCA结果中提取投影后的特征
  projected_features <- pca_result$x
