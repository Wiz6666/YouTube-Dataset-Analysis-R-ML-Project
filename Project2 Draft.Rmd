...
## 4.5 Descision Tree Model
### 4.5.1 Build Descision Tree Model
#### 4.5.1.1 Build Descision Tree Model for all features
...
cat('The AUC score on the training set is',tmodel_pre_train_auc, "\n")

#test set AUC score
tmodel_pre_test_auc <- AUC_calculator(predict(tree_model_pre, newdata=youtube_test), youtube_test[,target_column])
cat('The AUC score on the testing set is',tmodel_pre_test_auc)
```

### 4.5.2 Performance Measures
#### 4.5.2.1 Calculate the Model's accuracy, precision, recall, and f1 score
...
```
这个图像有点过度拟合，再重新试一下第二个模型，看看什么情况
print the performance table
```{r}
pretty_perf_table(tree_model_pre,youtube_train[c(categorical_independent_variables,numeric_independent_variables)],youtube_train[,target_column] == positive_label,youtube_test[c(categorical_independent_variables,numeric_independent_variables)],youtube_test[,target_column] == positive_label)
```
再看一下ROC图像
```{r}
