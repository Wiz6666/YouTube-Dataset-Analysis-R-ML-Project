...
本次Project，我们希望建立模型预测Youtuber的收入水平属于什么档次
我们选择使用的特征是："subs_engagement","uploads_frequencey","average_views_per_video","urban_rate","age","edu_ratio","top_channel_flag","video_views_increasing_rate",
类别是： 高收入 vs 低收入


  数据整合:
    从World Bank或其他公开数据源导入每个国家的GDP per capita数据。
    将Youtuber数据和GDP per capita数据根据国家字段进行合并。

  标准化Youtuber收入:
    使用Youtuber的年收入与其所在国家的GDP per capita进行比较。
    标准化收入=Youtuber年收入GDPpercapita标准化收入=GDPpercapitaYoutuber年收入​
    这可以给出Youtuber的收入是其国家平均收入的多少倍。

  定义高收入的逻辑:
    设定一个阈值。例如，如果Youtuber的标准化收入是国家GDP per capita的5倍或更多，则可以认为该Youtuber属于高收入群体。
    为什么选择5倍？这是一个启发式选择，可以基于数据的初步分析或行业常识进行选择。在初步分析中，您可以观察标准化收入的分布情况，查看哪些Youtuber位于高收入的尾部，从而为阈值选择提供依据。

  验证定义:
    一旦定义了高收入的逻辑，最好通过一些数据可视化来验证您的选择。例如，可以使用直方图或箱型图来查看标准化收入的分布，并检查高收入和低收入Youtuber的比例。

  调整和优化:
    根据验证步骤的结果，可能需要调整定义的逻辑。例如，如果您发现太多（或太少）的Youtuber被分类为高收入，您可能需要重新考虑和调整阈值。
...
youtube_data <- youtube_data[c("subs_engagement","uploads_frequencey","average_views_per_video","urban_rate","age","edu_ratio","top_channel_flag","video_views_increasing_rate","monthly_earning","yearly_earning")]
```

Use $12,535 to identify what means high income, and check the percentage of high income in dataset.
```{r Check the high income proportion}
# Define a function to calculate the proportion of high earners
calculate_high_earning <- function(col, threshold) {
  # Using vectorized comparison to obtain a logical vector for high and low earners
  high_logical <- col >= threshold  
  low_logical <- col < threshold     
  # Calculate the count of high and low earners using the sum function.
  # This works because in R, TRUE is treated as 1 and FALSE as 0.
  high_count <- sum(high_logical)    
  low_count <- sum(low_logical)      
  # Calculate the percentage of high and low earners
  high_earning_percentage <- high_count / length(col)  
  low_earning_percentage <- low_count / length(col)    
  # Return a named list with the calculated percentages
  return(list(high_earning_percentage = high_earning_percentage, 
              low_earning_percentage = low_earning_percentage))
}


# Call the function on the 'monthly_earning' column with a defined threshold of 12535
earnings_percentages <- calculate_high_earning(youtube_data$monthly_earning, 12535)
cat("There are", earnings_percentages$high_earning_percentage * 100, "% of YouTubers that belong to the high-income level.")

```

### 3.2.2 Handle earnings columns
