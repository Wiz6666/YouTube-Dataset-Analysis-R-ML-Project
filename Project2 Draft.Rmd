...
find_outliers <- detect_outliers(Youtube_cleaned[c(1:18)])
print(find_outliers)
```
...
```{r}
# 循环绘制每列数据的boxplot
for (col in colnames(Youtube_cleaned[c(1:18)])) {
  boxplot(Youtube_cleaned[, col], main=col)
}
...
truncate_tail <- function(data, column_names, percentile = 95) {
  for (col in column_names) {
    q <- quantile(data[[col]], probs = percentile / 100)
    data[data[[col]] > q, col] <- q
  }
  return(data)
}
modified_data <- truncate_tail(Youtube_cleaned, numeriCol, percentile = 95)
```
