...
### 3.1.6 Add new column which might be used in the future.
...
### 3.1.7 Handle outliers

#### 3.1.7.1 设置变量

先设置好 numeric 变量；category变量
```{r}
#筛选为category和numeric列
...
```
  ...
  In consideration of stakeholders and decision-makers who might be reviewing this report, it's essential to provide a clear visual representation of data discrepancies. Outliers can significantly impact the results of data analyses and predictive modeling. By visually inspecting these outliers using a boxplot, we ensure a better understanding of the data's distribution and its potential implications.
```{r Outliers boxplots,fig.width=16,fig.height=22}
long_data <- reshape2::melt(raw_data[numeric_vars_name], id.vars = NULL)

# Create a boxplot visualization for the data in the long format.
...
print(earnings_percentages$high_earning_percentage)
```
