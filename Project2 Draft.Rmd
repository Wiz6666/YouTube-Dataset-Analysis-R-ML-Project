...
#创建一个变量储存X(Independent variables) 
...
我们使用单一变量模型评估features。制作评估每个变量的Function
...
```{r category single variable model}
...
Single_variable_model <- function(target_column,feature_column, prediction_column){
  ...
  #当feature_column是NA时，计算高收入的分布
  na_table <- table(as.factor(target_column[is.na(feature_column)]))
  #当feature_column是NA时，使用上述的na_table 计算高收入为正的概率
  probility_of_positive_with_na <- (na_table/sum(na_table))[positive_label]
  
  #创建一个表格显示 feature_column不同取值 与target_column的关系
  value_table <- table(as.factor(target_column),feature_column)
  
  # 对于feature_column的每一个取值，计算target_variable为正类的条件概率
  probility_of_positive_with_val <- (value_table[positive_label, ] + 1.0e-3 * probility_of_positive) / (colSums(value_table) + 1.0e-3)
  
  # 使用上述概率进行预测
  predictions <- probility_of_positive_with_val[prediction_column]
  
  # 对于prediction_column中的NA值，使用probPosWithNA替代
  predictions[is.na(prediction_column)] <- probility_of_positive_with_na
  
  # 对于那些在训练数据中未出现，但可能在预测数据中出现的feature_column值，使用总体probPositive替代
...
```{r}
# 使用youtube_train数据集中的category特征预测earning_class列
predicted_probs <- Single_variable_model(
  target_column = youtube_train$earning_class, 
  feature_column = youtube_train$category, 
  prediction_column = youtube_train$category
)
...
for (var in categorical_independent_variables) {
    ...
    auc <- calcAUC(prediction_probs, youtube_train$earning_class)
    if (auc >= 0.1) {
...
# 例如，为第一个分类变量绘制ROC曲线（确保该变量AUC大于0.8）
var_to_plot <- categorical_independent_variables[1]
prediction_probs <- Single_variable_model(target_column = youtube_train$earning_class, 
...
SingleVariablePredictNumeric <- function(target_column, feature_column, prediction_column) {
    feature_discrete <- discretizeVariable(feature_column)
    prediction_discrete <- discretizeVariable(prediction_column)
    return(Single_variable_model(target_column, feature_discrete, prediction_discrete))
}
...
for (var in numeric_independent_variables) {
    ...
    auc_train <- calcAUC(youtube_train[, pred_col_name], youtube_train$earning_class)
    if (auc_train >= 0.55) {
