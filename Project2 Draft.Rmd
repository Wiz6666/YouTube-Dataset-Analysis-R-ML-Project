...
youtube_data <- youtube_data[
  c(
    ...
    "subs_engagement","uploads_frequencey","average_views_per_video","urban_rate","age","edu_ratio","top_channel_flag","video_views_increasing_rate","channel_age_adjusted_income"
    )
  ]
```
...
Independent varibale: "subscribers","video_views","uploads","channel_type","created_year","category","subs_engagement","uploads_frequencey","average_views_per_video"     "urban_rate","age","edu_ratio","top_channel_flag","video_views_increasing_rate","earning_class"   
...
### 4.4.1 Using Log likelihood and deviance
...
```{r warning=FALSE}
selVars <- c("channel_type","category","subscribers","video_views","uploads","created_year","subs_engagement","uploads_frequencey","average_views_per_video","urban_rate","age","edu_ratio","top_channel_flag","video_views_increasing_rate" )
...
##### d. Print the ROC plot
```{r}
pred_test_roc_tmodel_all_num <- predict(tree_model_pre, newdata=youtube_test)
pred_train_roc_tmodel_all_num <- predict(tree_model_pre, newdata=youtube_train)
...
### 4.5.3 Overfitting problem(这个可能运行有问题，要跳过这个)
Because the AUC of the decision tree model for all features is too high, we considered there might have a overfitting problem. Using learning curve to measure whether the model is overfitting or not.
```{r}
library(caret)
library(ggplot2)
...
# 定义要使用的分类算法（例如决策树）
model_name <- "rpart"
train_control <- trainControl(method = "cv", number = 5)  # 5折交叉验证

# 定义不同的训练集大小
train_sizes <- seq(0.1, 1.0, by = 0.1)

# 创建一个数据框来存储结果
learning_curve_data <- data.frame(TrainSize = numeric(0), Accuracy = numeric(0))

# 遍历不同的训练集大小
for (size in train_sizes) {
  # 从完整的youtube_train数据中创建一个子集
  subset_data <- createDataPartition(youtube_train$earning_class, p = size, list = FALSE)
  sample_data <- youtube_train[subset_data, ]
  
  # 使用交叉验证来评估模型性能
  results <- train(earning_class ~ ., data = sample_data, method = model_name, trControl = train_control)
  
  # 存储交叉验证的准确度
  accuracy <- results$results$Accuracy[1]
  
  # 存储结果
  learning_curve_data <- rbind(learning_curve_data, data.frame(TrainSize = size, Accuracy = accuracy))
}

# 绘制学习曲线
ggplot(learning_curve_data, aes(x = TrainSize, y = Accuracy)) +
  geom_line() +
  labs(title = "Learning Curve", x = "Training Data Size", y = "Accuracy") +
  theme_minimal()
